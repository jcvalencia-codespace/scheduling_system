...
export default function SectionHistory({ history, filters, activeTerm }) {
            ...
            <div key={idx} className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow space-y-3 border border-gray-200 dark:border-gray-700">
              ...
              <div className="space-y-2">
                <div>
                  ...
                  <div className="font-medium text-gray-900 dark:text-white">{entry.sectionDetails.name}</div>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="text-xs text-gray-500 dark:text-gray-400">Year Level:</label>
                    <div className="text-gray-900 dark:text-gray-300">{entry.sectionDetails.yearLevel}</div>
                  </div>
                  <div>
                    <label className="text-xs text-gray-500 dark:text-gray-400">Course:</label>
                    <div className="text-gray-900 dark:text-gray-300">{entry.sectionDetails.course}</div>
                  </div>
                </div>
                <div>
                  <label className="text-xs text-gray-500 dark:text-gray-400">Department:</label>
                  <div className="text-gray-900 dark:text-gray-300">{entry.sectionDetails.department}</div>
                </div>
              </div>
          ...
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {currentItems.map((entry, idx) => (
              <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">
                  {entry.sectionDetails.name}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">
                  {entry.sectionDetails.yearLevel}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">
                  {entry.sectionDetails.course}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">
                  {entry.sectionDetails.department}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">
                  <div className="flex flex-col">
                    <span className="font-medium text-gray-900 dark:text-white">{entry.updatedBy.name}</span>
