...
const generatePDF = (props, isPreview = false) => {
  ...
  timeSlots.forEach((time, timeIndex) => {
    weekDays.forEach((day, dayIndex) => {
      ...
      // Also check admin hours
      const adminHour = props.adminHours?.find(slot => 
        slot.day === day && 
        slot.status === 'approved' &&
        new Date(`2000/01/01 ${time}`).getTime() === new Date(`2000/01/01 ${slot.startTime}`).getTime()
      );
      
      if ((schedule || adminHour) && !scheduleSpans[`${timeIndex}-${dayIndex}`]) {
        if (adminHour) {
          let span = 0;
          for (let i = timeIndex; i < timeSlots.length; i++) {
            const currentTime = new Date(`2000/01/01 ${timeSlots[i]}`).getTime();
            const endTime = new Date(`2000/01/01 ${adminHour.endTime}`).getTime();
          ...
          scheduleSpans[`${timeIndex}-${dayIndex}`] = { 
            span, 
            isAdminHour: true,
            slot: {
              timeFrom: adminHour.startTime,
              timeTo: adminHour.endTime
            }
          };
        } else {
  ...
  doc.text("Class Schedule", 105, 25, { align: "center" });
  
  doc.setFontSize(8);
  doc.setTextColor(0);
  doc.text(`Schedule for: ${selectedSection}`, 105, 30, { align: "center" });
    ...
    weekDays.forEach((day, dayIndex) => {
      ...
      if (!spanInfo?.isSpanned) {
        ...
        if (spanInfo?.span) {
          ...
          // Different content for admin hours vs regular schedules
